...
-- 1. 서브쿼리 작성
select count(*) as 'count' from tbl_menu group by category_code; 
-- max() 함수 : 최대값, min() 함수 : 최솟값
-- from 절에 사용하는 서브쿼리는 (derived table,menu_codemenu_namemenu_pricecategory_codeorderable_status 파생 테이블) 이라고 한다.
-- 파생 테이블은 반드시 별칭을 가지고 있어야 한다.
select max(count) from (select count(*) as 'count' from tbl_menu group by category_code) as countmenu;
...
with menucte as (select menu_name, category_name from tbl_menu a join tbl_category b on a.category_code = b.category_code)
select * from menucte order by menu_name asc;
