...
-- as(alias) 별칭
-- sql 문(쿼리문) 의 컬럼또는 테이블에 별칭을 달 수 있다.
...
-- inner join
-- 두테이블의 교집합을 반환하는 join유형
-- inner join 에서 inner 는 생략이 가능하다.

-- on 키워드를 사용한 join 방법
-- 컬럼명이 같거나, 다른경우 on 으로 서로 연관있는 컬럼에 대한 조건 작성
-- 메뉴의 이름과 카테고리 이름의 조회
...
select
    ...
    b.category_name
from
	tbl_menu a -- 카테고리 변수를 join을 이용해 변수 만들기
    join
    tbl_category b 
    on 
    ...
    -- using 을 사용한 join
    -- 서로 다른 두테이블에서 공유하고 있는 컬럼명이 동일한 경우
...
-- left join
-- 첫번째 테이블의 모든 레코드와 두 번째 테이블에서 일치하는
-- 레코드를 반환하는 join 유형
...
-- cross join 은 가능한 모든 조합을 반환하는 합집합의 개념
...
-- 서로 다른 테이블 간의 데이터를 조회하고 싶을 때 join 을 사용한다.
-- join 을 수행하는 단계를 고려할때는
